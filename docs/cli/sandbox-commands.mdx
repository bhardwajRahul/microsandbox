...
Change a running sandbox's configuration. Every change is planned, labeled (live, next start, requires restart, unsupported), and applied all-or-nothing. CPU and memory resize live within the `max_cpus` / `max_memory` ceilings; raising a ceiling requires `--restart` or `--next-start`.
