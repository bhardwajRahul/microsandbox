...
`msb update`, `msb upgrade`, and `msb downgrade` are now top-level aliases for the existing `msb self update` and `msb self downgrade` flows, matching the shorter commands people already reach for from other package managers. See the [CLI overview](/cli/overview).
